...
public class RunTrajectoryCommandSwerve extends CommandBase {
    ...
    public RunTrajectoryCommandSwerve(SwerveDriveServo drive, Trajectory traj, CommandOpMode opMode) {
        this.drive = drive;
        this.traj = traj;
        myOpmode = opMode;
...
//        myOpmode.telemetry.addData("CURSTATE_IMU", driveSim.getHeading().getDegrees());
        ...
        ChassisSpeeds targetChassisSpeeds =
                m_holonomicController.calculate(drive.getPose(), desiredState, desiredRotation);
        SwerveModuleState[] targetModuleStates = SwerveDriveConstants.swerveKinematics.toSwerveModuleStates(targetChassisSpeeds);

        for (SwerveModuleServo module : drive.modules) {
